import { Chapter } from "./store/features/chapters/chaptersSlice"
...
const getRandomIcon = () => {
  return phosphorIcons[Math.floor(Math.random() * phosphorIcons.length)]
}
...
// Helper function to get total questions for a chapter
export const getTotalQuestions = (yearWiseQuestionCount: YearWiseQuestionCount): number => {
  return Object.values(yearWiseQuestionCount).reduce((acc, count) => acc + count, 0)
}

// Helper function to get progress percentage
export const getProgressPercentage = (questionsSolved: number, totalQuestions: number): number => {
  return totalQuestions > 0 ? Math.round((questionsSolved / totalQuestions) * 100) : 0
}

// Helper function to format year-wise questions
export const formatYearWiseQuestions = (yearWiseQuestionCount: YearWiseQuestionCount): string => {
  const latestYears = ["2025", "2024"]
  return latestYears
    .map(year => {
      const count = yearWiseQuestionCount[year] || 0
      return `${year}: ${count}s${count > 0 ? " ↑" : ""}`
    })
    .join(" | ")
} 